...
この拡張は PractiCase 本体の一部であり、単独のライセンスは持ちません。
利用条件は、リポジトリ直下の README.md のライセンス関連の節(見出しは版により異なる)が正本です。
...
2. 初回は**エクスプローラーに「クエストログ」セクションが自動で表示される**
   (2回目以降は画面下の **🎓** をクリック、または Ctrl+Shift+P → 「PractiCase」)
3. 「チュートリアルを始める」→ 以降はログの案内どおり。ログはファイルツリーと同じ
   エクスプローラー内に居座るので、作業ファイルをいくら開いてもログは隠れない。
   **「できた、次へ」は画面下にも常駐**しているので、ログを見ていなくても進められる
