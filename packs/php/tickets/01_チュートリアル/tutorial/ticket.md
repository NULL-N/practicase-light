...
左の 🎓 パネルが VS Code 内の手順 — ファイルの場所・status の変え方・検索・check・報告 — も1歩ずつ案内します。

## やること(この順で)

1. この ticket.md の `status: open` を `status: in_progress` に書き換えて保存する
   - あわせて「何分で終わりそうか」を一言見積もっておく(頭の中でよい。終わったとき見比べる —
     実務でずっと使う習慣の種)
...
7. この ticket.md の status を `closed` にする — これで1周完了です
