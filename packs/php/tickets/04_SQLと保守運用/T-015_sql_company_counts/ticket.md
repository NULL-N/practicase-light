...
1. Redmineで`PractiCase Ticket ID`が`T-015`のチケットを開き、担当者を自分にして、
...
2. VS Codeで教材フォルダを開いた状態のターミナルから、mainを更新して作業branchを作る
   ...
   git pull --ff-only
   git status --short
   git switch -c feature/redmine-<チケット番号>-T-015-sql-company-counts
   ```

`<チケット番号>`はRedmineのURL末尾の数字です。URLが`/issues/3`なら、branch名は
`feature/redmine-3-T-015-sql-company-counts`です。`git status --short`に何か表示された場合は、branchを作る前に変更内容を確認します。
...
5. `support/debrief/`がある課題は、Pull Requestをmergeした後に開いて自分の提出と突き合わせる。
   ...
   `reports/T-015_retrospective.md`を使用する
...
> branch名は`feature/T-015-<短い名前>`へ読み替えます。Redmineとの自動同期はありません。
