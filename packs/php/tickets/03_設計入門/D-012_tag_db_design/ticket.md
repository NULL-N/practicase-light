...
> - **今回は DB 設計の必須要素(主キー・外部キー・NOT NULL・重複防止のユニーク制約・
>   created_at/updated_at)を check でも見ます。** 書き方(Markdown 表でも SQL の
>   CREATE TABLE でもよい)は自由ですが、これらの要素が抜けているとテーブルとして
>   欠陥があるとみなし、check は FAIL にします。FAIL メッセージには「なぜ必要か」も書いてあります
