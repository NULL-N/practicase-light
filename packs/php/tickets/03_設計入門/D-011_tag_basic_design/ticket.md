...
> - **新しい画面は増やしません。** ただし、検索条件が1つ増えるので、4節の画面遷移図
>   (Mermaid)にも「タグで絞り込む」という流れを一言分だけ足してください。本格的な遷移図の
>   書き直しは不要です — 既存の図に1行足すだけで十分です
...
5. `docs/01_設計資料/screens.md` の4節(画面遷移図)の Mermaid に、タグ絞り込みの流れを一言追記する
6. check:
...
- `check D-011` が PASS(F-20 の節・画面側への追記・画面遷移図へのタグ絞り込み追記が存在すること)
