...
PractiCase Lightでは、Redmineで取り組むチケットを確認し、
担当者・ステータス・作業コメントを記録します。
課題内容は`ticket.md`、提出物は`reports/`が正本です。

Redmineの英語画面やREST APIでは、チケットを`issue`と表記します。
このガイドと教材の説明文では「チケット」に統一します。

## まずこれだけ

- **初回**: Redmineを起動 → 初回セットアップ → 全課題を登録
- **普段**: チケット確認 → In Progress → ブランチ作成 → 作業 → Closed
- **終了時**: Redmineを停止

Redmineが使えない場合だけ、`ticket.md`のfront matterを使うfallbackへ切り替えます。
...
4. チケットのステータスを **New → In Progress** に変更する
5. ターミナルで、このチケット用の作業ブランチを作る
6. `ticket.md` の指示に従ってコードを書き、`support/` の資料を読み、
   `docker compose exec app php tools/check.php <課題ID>` を実行する
7. check が PASS したら、結果と提出物名をコメントに残し、status を
   **In Progress → Resolved** に変更する
8. 提出(PR 作成・reports への記載など)とセルフレビューまで終えたら
   **Resolved → Closed** にして完了する

ブランチ名は次の形式にします。

`feature/redmine-<チケット番号>-<課題ID>-<短い名前>`

例:

`feature/redmine-3-T-018-login-investigation`

- `3`: Redmineのチケット番号
- `T-018`: 課題ID
- `login-investigation`: 課題内容を表す短い名前

短い名前は、原則として課題フォルダ名の課題IDより後ろを使います。
`T-018_login_investigation`なら、`_`を`-`へ変えて
`login-investigation`とします。

commit・push・Pull Requestを含む詳細は、
[workflow.mdの「ブランチとコミットのルール」](workflow.md#ブランチとコミットのルール)
を参照してください。
...
<details>
<summary>PowerShellのreset手順を開く</summary>
...
<details>
<summary>Git Bashのreset手順を開く</summary>
...
チケットの詳細画面には、変更の履歴(journal)が時系列で並びます。ここには2種類が
混ざることがあります:
...
確認したいときは、チケット詳細画面の履歴をそのまま上から読んでください(検索や
フィルタは不要な規模です)。
...
- **status を変更したのに反映が変(New のまま等)**: ごく稀に、チケット作成直後の
  status 遷移が失敗することがあります。この場合、seed の再実行では直りません
  (通常の seed は既存チケットの status に触れない設計のため)。
  「PractiCase Ticket ID」でそのチケットを特定し、Redmine の画面上で手動で
...
この教材ではRedmine `6.1.3`とPostgreSQL `14.23`を固定しています。
Docker Composeが固定済みのイメージを取得して起動するため、RedmineやPostgreSQLを
ホストへ個別にインストールする必要はありません。

最終確認日: 2026-07-19

確認時点では、Redmine本体の最新安定版は`7.0.0`ですが、Redmine 7の
Docker公式イメージは未公開です。今すぐ7系を使うには教材独自のDockerイメージを
継続保守する必要があるため、検証済みのDocker公式最新版`6.1.3`を採用しています。

PostgreSQL `14.23`はPostgreSQL全体の最新版ではなく、Redmineの公式互換性表に
記載された14系の最新minorです。PostgreSQL 14の公式サポート終了予定日は
2026-11-12です。

Redmine 7のDocker公式イメージ公開時、またはPostgreSQL 14のサポート終了前に、
Redmineが公式に推奨するサポート中のPostgreSQLとの組み合わせを再検証し、
使用するイメージのdigestを固定し直します。

公式の確認先:

- [Redmine Docker Official Image](https://hub.docker.com/_/redmine/)
- [Redmineの対応環境](https://www.redmine.org/projects/redmine/wiki/RedmineInstall)
- [PostgreSQLのサポート期間](https://www.postgresql.org/support/versioning/)
