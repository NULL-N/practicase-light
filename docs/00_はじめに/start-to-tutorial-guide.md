...
| Redmine | bootstrapと全33課題のseedが完了し、学習者でログインできる |
| T-000 | `check T-000` が PASS、完了報告が `reports/` にあり、Redmine issueがClosed |
| tutorial | `check tutorial` が PASS、3行報告が `reports/` にあり、Redmine issueがClosed |
| tutorial-2 | `check tutorial-2` が PASS、3行報告が `reports/` にあり、Redmine issueがClosed |
...
Redmineの初期設定と全33課題の投入を行います。どちらも再実行できます。
...
プロジェクト「PractiCase Light」に33件のissueが表示されれば準備完了です。
...
1. Redmineで`PractiCase Ticket ID`が`T-000`のissueを開き、自分を担当者にして
   **New → In Progress**にする
2. 起動確認を行い、見積や気づきをissueのコメントへ書く
...
`結果: PASS` になったら、issueへ`check T-000: PASS`と報告ファイル名をコメントし、
...
Redmineで`PractiCase Ticket ID`が`tutorial`のissueを開き、自分を担当者にして
**New → In Progress**にします。作業前の見積もコメントへ残します。
...
最後にissueへ`check tutorial: PASS`と報告ファイル名をコメントし、
**In Progress → Resolved → Closed**にします。
...
Redmineで`PractiCase Ticket ID`が`tutorial-2`のissueを開き、自分を担当者にして
**New → In Progress**にします。前回の実績と比べた見積をコメントへ残します。
...
最後にissueへ`check tutorial-2: PASS`と報告ファイル名をコメントし、
**In Progress → Resolved → Closed**にします。
...
- [ ] `reports/` に3本の報告がある
- [ ] Redmineで`T-000`、`tutorial`、`tutorial-2`のissueがClosed
